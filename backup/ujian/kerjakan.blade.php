<x-app-layout>

    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            Ujian: {{ $soal->nama_ujian }}
        </h2>
    </x-slot>

    <div class="px-4 py-6">

        <div class="flex max-w-6xl mx-auto gap-4">

            {{-- ===========================
                PANEL DAFTAR NOMOR SOAL
            ============================ --}}
            <div class="w-1/4 bg-white shadow-md rounded-lg p-4 h-fit sticky top-4">
                <h3 class="font-semibold mb-3">Daftar Soal</h3>

                <div class="grid md:grid-cols-5 gap-2">

                    @foreach ($nomorList as $i => $questId)
                        @php
                            // Ambil riwayat jawaban user
                            $riwayatItem = \App\Models\RiwayatUjian::where('user_id', auth()->id())
                                ->where('soal_id', $soal->id)
                                ->where('quest_id', $questId)
                                ->first();

                            // Tentukan warna kotak
                            if ($i+1 == $no) {
                                $warna = 'bg-green-500 text-white'; // aktif
                            } elseif ($riwayatItem && $riwayatItem->jawaban) {
                                $warna = 'bg-blue-600 text-white'; // sudah dijawab
                            } else {
                                $warna = 'bg-white text-gray-500'; // belum dijawab
                            }
                        @endphp

                        <a href="{{ route('ujian.kerjakan', ['soal' => $soal->id, 'no' => $i+1]) }}"
                           class="w-10 h-10 border border-gray-700 flex items-center justify-center rounded font-bold {{ $warna }}">
                            {{ $i+1 }}
                        </a>
                    @endforeach

                </div>
            </div>

            {{-- ===========================
                PANEL UTAMA SOAL
            ============================ --}}
            <div class="w-3/4 p-6 bg-white shadow-md rounded-lg">

                <div id="timer" class="text-xl font-bold text-red-600 mb-4"></div>

                <h3 class="mb-4 text-lg font-semibold">
                    Soal {{ $no }} dari {{ $totalSoal }}
                </h3>

                <form id="formUjian">
                    @csrf
                    <input type="hidden" id="soal_id" value="{{ $soal->id }}">
                    <input type="hidden" id="quest_id" value="{{ $quest->id }}">

                    <div class="mb-4">
                        {!! $quest->soal !!}
                    </div>

                    <div class="space-y-2">
                        @php
                            $jawabanSebelumnya = $riwayat->jawaban;
                        @endphp

                        @foreach (['A','B','C','D','E'] as $opsi)
                            @php $field = "opsi_" . strtolower($opsi); @endphp
                            @if ($quest->$field)
                                <label class="flex items-center gap-2">
                                    <input type="radio"
                                           name="jawaban"
                                           value="{{ $opsi }}"
                                           @checked($jawabanSebelumnya == $opsi)>
                                    <span>{{ $opsi }}. {!! $quest->$field !!}</span>
                                </label>
                            @endif
                        @endforeach
                    </div>

                </form>

                <div class="flex justify-between mt-6">
                    {{-- Tombol Back --}}
                    @if ($no > 1)
                        <a href="{{ route('ujian.kerjakan', ['soal' => $soal->id, 'no' => $no - 1]) }}"
                           class="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700">
                            ← Sebelumnya
                        </a>
                    @endif

                    {{-- Tombol Next --}}
                    @if ($no < $totalSoal)
                        <a href="{{ route('ujian.kerjakan', ['soal' => $soal->id, 'no' => $no + 1]) }}"
                           class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                            Selanjutnya →
                        </a>
                    @endif

                    {{-- Tombol Submit --}}
                    @if ($no == $totalSoal)
                        <form action="{{ route('ujian.submit', $soal->id) }}" method="POST">
                            @csrf
                            <button class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">
                                Submit Ujian
                            </button>
                        </form>
                    @endif
                </div>

            </div>
        </div>
    </div>

    {{-- ===========================
        SCRIPT AUTOSAVE
    ============================ --}}
    <script>
        document.querySelectorAll('input[name=jawaban]').forEach(radio => {
            radio.addEventListener('change', function () {
                fetch("{{ route('ujian.autosave') }}", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": "{{ csrf_token() }}"
                    },
                    body: JSON.stringify({
                        soal_id: document.getElementById("soal_id").value,
                        quest_id: document.getElementById("quest_id").value,
                        jawaban: this.value
                    })
                }).catch(err => console.error("Autosave gagal:", err));
            });
        });
    </script>

    {{-- ===========================
        SCRIPT TIMER
    ============================ --}}
    <script>
        const waktuMulai = new Date("{{ $ujianSiswa->waktu_mulai }}").getTime();
        const durasiUjian = {{ $soal->durasi }} * 60 * 1000;

        const timerDiv = document.getElementById("timer");

        function updateTimer() {
            const now = new Date().getTime();
            const selisih = now - waktuMulai;
            const sisa = durasiUjian - selisih;

            if (sisa <= 0) {
                timerDiv.textContent = "Waktu Habis!";
                window.location.href = "{{ route('ujian.submit', $soal->id) }}";
                return;
            }

            let menit = Math.floor(sisa / 1000 / 60);
            let detik = Math.floor((sisa / 1000) % 60);

            if (detik < 10) detik = '0' + detik;

            timerDiv.textContent = `${menit}:${detik}`;
        }

        setInterval(updateTimer, 1000);
        updateTimer();
    </script>

    {{-- ===========================
        BLOKIR TOMBOL BACK BROWSER
    ============================ --}}
    <script>
        history.pushState(null, "", location.href);
        window.onpopstate = function () {
            history.pushState(null, "", location.href);
            alert("Tidak dapat kembali! Ujian sedang berlangsung.");
        };
    </script>

</x-app-layout>
