...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function applyFilters() {
            ...
            const hari   = filterHari.value.toLowerCase();
            const mapel  = filterMapel.value.toLowerCase();
            const kelas  = filterKelas.value.toLowerCase();

            // Loop hanya row yang ada di tbody
            table.querySelectorAll('tbody tr').forEach(row => {
                // Pastikan row bukan row kosong
                if (!row.querySelectorAll('td').length) return;

                const rowHari   = row.cells[0].textContent.toLowerCase();
                const rowMapel  = row.cells[2].textContent.toLowerCase();
                const rowKelas  = row.cells[3].textContent.toLowerCase();
                const rowText   = row.textContent.toLowerCase();

                const visible =
                    (search === "" || rowText.includes(search)) &&
                    (hari === ""   || rowHari.includes(hari)) &&
                    (mapel === ""  || rowMapel.includes(mapel)) &&
                    (kelas === ""  || rowKelas.includes(kelas));
        ...
        [searchInput, filterHari, filterMapel, filterKelas].forEach(el => {
        ...
        document.getElementById('exportPDF').addEventListener('click', () => {
            ...
            console.log("Base64 mime:", mime);
            console.log("Base64 length:", sekolah.logo_base64.length);
            console.log("Preview:", imgData.substring(0,100) + "...");
            ...
            tengahY += 5;
            doc.setFont('times', 'normal'); doc.setFontSize(13); doc.setTextColor(0,180,0);
            doc.text('BISNIS MANAJEMEN TERAKREDITASI "B"', tengahX, tengahY);
            tengahY += 5; doc.setFontSize(8.8);
            ...
            // doc.text("JADWAL MATA PELAJARAN", pageWidth/2, yTableTitle,{align:'center'});
            ...
            // autoTable
            doc.autoTable({
                head: [["Hari","Jam","Nama Mapel","Kelas"]],
                body: tableRows,
                ...
                columnStyles: { 0:{halign:'center'}, 2:{halign:'left'}, 3:{halign:'left'} }
            });
