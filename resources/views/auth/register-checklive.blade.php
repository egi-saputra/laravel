<x-guest-layout>
    ...
    <div class="flex flex-col min-h-screen md:flex-row">
        ...
        <div class="flex flex-col items-center justify-center w-full p-8 text-center bg-slate-50 md:w-1/2">
            <img src="{{ Storage::url('logo_login/bg_login.png') }}" alt="Logo" class="w-1/2 mb-4">
            <h3 class="mt-4 mb-3 text-[#063970] text-xl md:text-2xl font-semibold">
                SIMSTAL | {{ $profil->nama_sekolah ?? 'Nama Sekolah Belum Diset' }}
            </h3>
            {{-- <p class="px-4 text-[#063970] text-sm md:text-base">
                Bergabunglah bersama kami, kelola progress administrasi, pembelajaran, pekerjaan dan lainnya bersama kami. Kami memberikan kemudahan dalam akses dan pelayanan.
            </p> --}}
            <p class="px-4 text-[#063970] mb-2 text-sm md:text-base">
            ...
            <div class="w-full max-w-md">
                ...
                <form method="POST" action="{{ route('register') }}" class="w-full max-w-md mx-auto">
                    ...
                    <div class="relative mb-6">
                        ...
                        <!-- Checklist Rules -->
                        <ul class="mt-2 space-y-1 text-xs text-gray-600" id="password-rules">
                            <li id="rule-length" class="flex items-center">Minimal 6 karakter</li>
                            <li id="rule-lower" class="flex items-center">Ada huruf kecil</li>
                            <li id="rule-upper" class="flex items-center">Ada huruf besar</li>
                            <li id="rule-number" class="flex items-center">Ada angka</li>
                            <li id="rule-symbol" class="flex items-center">Ada simbol</li>
                        </ul>
                    ...
                    <div class="mb-3">
                        <button id="registerBtn" type="submit" disabled
                                class="w-full py-2 font-semibold text-white transition-colors rounded-full cursor-not-allowed bg-slate-400">
                            REGISTER
                        </button>
                    </div>

                    <!-- Terms of service and Privacy Policy -->
                    <div class="flex items-center justify-center mb-3 text-sm font-semibold text-slate-600 ">
    ...
    passwordInput.addEventListener('input', () => {
        ...
        if (hasLower && hasUpper && hasNumber && hasSymbol && hasLength) {
            ...
            registerBtn.classList.add('bg-slate-900', 'hover:bg-slate-800');
        } else {
            registerBtn.disabled = true;
            registerBtn.classList.remove('bg-slate-900', 'hover:bg-slate-800');
    ...
    function updateRule(id, valid) {
        ...
        if (valid) {
            if (!el.querySelector('.checkmark')) {
                ...
                span.textContent = '✔';
                span.classList.add('checkmark', 'mr-1', 'text-green-600');
                el.prepend(span);
            }
